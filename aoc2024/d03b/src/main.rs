...
impl<'a> Parser<'a> {
    ...
    fn parse_stream(&mut self) -> ParseResult {
        ...
        loop {
            ...
            match ValidChars::parse(token) {
                Some(LetterD) => {
                    if let Some(do_it) = self.parse_do() {
                        enabled = do_it;
                    } else {
                        self.next();
    ...
    // true is do, false is dont
    fn parse_do(&mut self) -> Option<bool> {
        if self.token.is_none() {
            return None;
        }

        let mut word = vec![ValidChars::LetterD];
        while let Some(token) = self.next() {
            match ValidChars::parse(token) {
                Some(ValidChars::LetterO) if word.len() == 1 => word.push(ValidChars::LetterO),
                Some(ValidChars::LetterN) if word.len() == 2 => word.push(ValidChars::LetterN),
                Some(ValidChars::Apostrophe) if word.len() == 3 => {
                    word.push(ValidChars::Apostrophe)
                }
                Some(ValidChars::LetterT) if word.len() == 4 => word.push(ValidChars::LetterT),
                Some(ValidChars::LeftParen) if word.len() == 5 || word.len() == 2 => {
                    word.push(ValidChars::LeftParen)
                }
                Some(ValidChars::RightParen) if word.len() == 6 || word.len() == 3 => {
                    word.push(ValidChars::RightParen)
                }
                _ => break,
            }
        }
        ...
        if word
            == vec![
                ...
                ValidChars::LetterO,
                ValidChars::LetterN,
                ValidChars::Apostrophe,
                ValidChars::LetterT,
                ValidChars::LeftParen,
                ValidChars::RightParen,
            ]
        {
            Some(false)
        } else if word
...
impl std::fmt::Display for ValidChars {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ValidChars::LetterD => write!(f, "d"),
            ValidChars::LetterO => write!(f, "o"),
            ValidChars::LetterN => write!(f, "n"),
            ValidChars::LetterT => write!(f, "t"),
            ValidChars::LeftParen => write!(f, "("),
            ValidChars::RightParen => write!(f, ")"),
            ValidChars::Apostrophe => write!(f, "'"),
...
impl ValidChars {
    fn parse(c: char) -> Option<ValidChars> {
        match c {
            ...
            'D' | 'd' => Some(ValidChars::LetterD),
            'O' | 'o' => Some(ValidChars::LetterO),
            'N' | 'n' => Some(ValidChars::LetterN),
            'T' | 't' => Some(ValidChars::LetterT),
            '\'' => Some(ValidChars::Apostrophe),
